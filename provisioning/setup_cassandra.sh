...
sudo sed -i "s/seeds: \"127.0.0.1\"/seeds: \"10.2.5.170,10.2.5.171,10.2.6.170,10.2.6.171\"/g" /etc/cassandra/cassandra.yaml
...
if [ $node == "0" ]; then
    ...
    sudo sed -i "s/# broadcast_address: 1.2.3.4/broadcast_address: 10.3.6.170/g" /etc/cassandra/cassandra.yaml
elif [ $node = "4" ]; then
    sudo sed -i "s/# broadcast_address: 1.2.3.4/broadcast_address: 10.3.6.171/g" /etc/cassandra/cassandra.yaml
elif [ $node = "5" ]; then
    sudo sed -i "s/# broadcast_address: 1.2.3.4/broadcast_address: 10.3.6.172/g" /etc/cassandra/cassandra.yaml
else
    echo "$HELP"
    exit 1
fi
